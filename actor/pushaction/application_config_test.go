...
var _ = Describe("Application Config", func() {
	...
	Describe("ConvertToApplicationConfigs", func() {
		...
		Context("when routes are not defined", func() {
			...
			FContext("when only the -d flag is provided", func() {
				BeforeEach(func() {
				...
				Context("when the provided domain exists", func() {
					BeforeEach(func() {
						fakeV2Actor.GetDomainsByNameAndOrganizationReturns(
							[]v2action.Domain{domain},
							v2action.Warnings{"some-private-domain-warning"},
							nil,
						)
					})

					FIt("it uses the provided domain instead of the first shared domain", func() {
						Expect(executeErr).ToNot(HaveOccurred())
						Expect(warnings).To(ConsistOf("some-private-domain-warning", "private-domain-warnings", "shared-domain-warnings", "app-route-warnings"))
				...
				PContext("when the provided domain does not exist", func() {
