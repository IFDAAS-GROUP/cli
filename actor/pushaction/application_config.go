...
func (actor Actor) configureRoutes(manifestApp manifest.Application, orgGUID string, spaceGUID string, config ApplicationConfig) (ApplicationConfig, Warnings, error) {
	...
	if len(manifestApp.Routes) > 0 {
		config.DesiredRoutes, warnings, err = actor.CalculateRoutes(manifestApp, orgGUID, spaceGUID, config.CurrentRoutes)
		return config, warnings, err
	}

	// TODO: Rename GetRouteWithDefaultDomain to GetRouteWithDesiredDomain
	desiredRoute, warnings, err := actor.GetRouteWithDesiredDomain(manifestApp, config.DesiredApplication.Name, orgGUID, spaceGUID, config.CurrentRoutes)
	if err != nil {
